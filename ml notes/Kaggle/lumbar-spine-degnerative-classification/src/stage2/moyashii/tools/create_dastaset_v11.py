...
DST_DIR = SETTINGS.train_data_clean_dir / f'{args.user}/classification_datasets/v11'
DST_DIR.mkdir(exist_ok=True, parents=True)

# 切り出し
SAGITTAL_CROP_MM = [[21.5, 14.5], [22.5, 22.5]]  # [top=21.5mm, bottom=14.5mm], [left=22.5mm, right=22.5mm]
SAGITTAL_CROP_OFFSET_MM = -5  # 法線ベクトル方向にmm単位でオフセットする
AXIAL_CROP_MM = [[17.5, 27.5], [37.5, 37.5]]  # [top=17.5mm, bottom=27.5mm], [left=37.5mm, right=37.5mm]
        ...
        for series_id in desc_to_series_id[desc]:
            ...
            for i in list(range(num_slices)):
                ...
                for level, position in sagittal_image_norm_coord.items():
                    ...
                    crop_image, rect_x, rect_y = extract_rectangle_from_single_point(image,
                                                                                     x_smooth, y_smooth,
                                                                                     interp_distances,
                                                                                     x, y,
                    ...
                    if np.any(rect_x < 0) or np.any(rect_x >= image_shape[1]) or np.any(rect_y < 0) or np.any(rect_y >= image_shape[0]):
                        print(f'{study_id} {series_id} {level} {desc} {stack["instance_numbers"][i]} is out of image')
                    ...
                    crop_data.append({
                        ...
                        'norm_x': position[0],
                        'norm_y': position[1],
                        'rect': np.array([rect_x, rect_y]).T.tolist(),
            ...
            for level, stack_indices in stack_indices_per_level.items():
                ...
                for i in stack_indices:
                    ...
                    crop_data.append({
                        ...
                        'x': x,
                        'y': y,
                        'norm_x': axial_image_norm_coord['center'][0],
                        'norm_y': axial_image_norm_coord['center'][1],
...
CROP_DF.to_csv(DST_DIR / 'crop_data.csv', index=False)
...
def is_point_in_rectangle(rect: np.ndarray, q: np.ndarray) -> bool:
    ...
    # 各ベクトルの外積を計算
    cp1 = cross_product(v1, qp1)
    cp2 = cross_product(v2, qp2)
    cp3 = cross_product(v3, qp3)
    cp4 = cross_product(v4, qp4)

    # 外積の符号がすべて同じかどうかを確認、同じならば点は矩形内にある
...
for study_id in tqdm(study_ids, total=len(study_ids)):
    ...
    for series_id in study_crop_df['series_id'].unique():
        ...
        for i, gt_row in series_gt_coord_df.iterrows():
            data = []
            for i, crop_row in instance_crop_df.iterrows():
                in_crop = is_point_in_rectangle(np.array(crop_row['rect']), np.array([gt_row['x'], gt_row['y']]))
                data.append(dict(in_crop=in_crop, level=crop_row['level']))
            in_crop_df = pd.DataFrame(data)

            # クロップした位置に正しい椎間板レベルのGTが含まれているならTP
            tp += int(np.all(in_crop_df[in_crop_df['level'] == gt_row['level']]['in_crop']))
            # クロップした位置に正しくない椎間板レベルのGTが含まれているならFP
            fp += int(np.any(in_crop_df[in_crop_df['level'] != gt_row['level']]['in_crop']))
            total += 1

print(f'クロップ領域に正しい椎間板レベルのGTが含まれる確率={tp / total * 100:.2f}% ({tp}/{total})')
print(f'クロップ領域に誤った椎間板レベルのGTが含まれる確率={fp / total * 100:.2f}% ({fp}/{total})')
...
print(
    f"アキシャルスライスの取りこぼし確率={(1 - crop_analysis_df['exist'].sum() / len(crop_analysis_df)) * 100:.2f}% ({crop_analysis_df['exist'].sum()}/{len(crop_analysis_df)})")
...
for study_id in tqdm(study_ids, total=len(study_ids)):
    ...
    for series_id in study_crop_df['series_id'].unique():
        ...
        for _, gt_row in series_gt_coord_df.iterrows():
            instance_number = gt_row.instance_number
            instance_crop_df = series_crop_df[series_crop_df['instance_number'] == instance_number]
            if len(instance_crop_df) != 1:
                # クロップした数が1でない＝正しく椎間板レベル割り当て割れていない。スキップする
                continue

            crop_row = instance_crop_df.iloc[0]
            in_crop = is_point_in_rectangle(np.array(crop_row['rect']), np.array([gt_row['x'], gt_row['y']]))
            # クロップした位置に正しい椎間板レベルのGTが含まれているならTP
            tp += int(in_crop and crop_row['level'] == gt_row.level)
            # クロップした位置に正しくない椎間板レベルのGTが含まれているならFP
            fp += int(in_crop and crop_row['level'] != gt_row.level)
            total += 1
